...
describe('DailyGameService — starting a game (integration)', () => {
    ...
    it('starts a new game end-to-end: real fetch call, store updated, cookie persisted', async () => {
        server.use(
            http.post('*/game/daily/start', () => HttpResponse.json(MOCK_DAILY_START_GAME_ID)),
        );

        const gameId = await new DailyGameService(
            new DailyGameApi(),
            useGameStore(),
        ).startNewGame();

        expect(gameId).toBe(MOCK_DAILY_START_GAME_ID);
        expect(useGameStore().game).toMatchObject({
            gameId: MOCK_DAILY_START_GAME_ID,
...
describe('DailyGameService — making a guess (integration)', () => {
        ...
        server.use(
            http.post('*/game/daily/guess', async ({ request }) => {
                ...
                return HttpResponse.json(
                    buildGuessResponse({ monsterCode: MONSTER_CODE_NARGACUGA }),
                );
            }),
        );

        await new DailyGameService(new DailyGameApi(), useGameStore()).makeGuess(
            SHAPE_TEST_GAME_ID,
            MONSTER_CODE_NARGACUGA,
        );
    ...
    it('sends the guess and records the result in the store', async () => {
        server.use(
            http.post('*/game/daily/guess', () => {
                return HttpResponse.json(
                    buildGuessResponse({ gameStateAfterGuess: GameStates.Ongoing }),
                );
            }),
        );
    ...
    it('propagates a Win state from the response to the store', async () => {
        server.use(
            http.post('*/game/daily/guess', () => {
                return HttpResponse.json(
                    buildGuessResponse({ gameStateAfterGuess: GameStates.Win }),
                );
            }),
        );
    ...
    it('propagates a backend error instead of silently recording a guess', async () => {
        ...
        await expect(
            gameService.makeGuess(EXISTING_GAME_ID, MONSTER_CODE_RATHALOS),
        ).rejects.toThrow();
...
describe('DailyGameService — resuming a game (integration)', () => {
    ...
    it('requests the correct game id and method in the URL', async () => {
        ...
        await new DailyGameService(new DailyGameApi(), useGameStore()).resumeGame(
            SHAPE_TEST_GAME_ID,
        );
    ...
    it('resumes an ongoing game and reflects it in the store', async () => {
        ...
        const isOngoing = await new DailyGameService(new DailyGameApi(), useGameStore()).resumeGame(
            EXISTING_GAME_ID,
        );
    ...
    it('persists an already-finished game, and reports it as resumed', async () => {
        ...
        const isGameSet = await new DailyGameService(new DailyGameApi(), useGameStore()).resumeGame(
            EXISTING_GAME_ID,
        );
    ...
    it('returns false and leaves the store empty when the backend has no matching game', async () => {
        server.use(
            http.get('*/game/daily/resume/:gameId', () => new HttpResponse(null, { status: 404 })),
        );
