        ...
        <div class="guess-table" :class="getA11yClasses()" role="table" v-if="hasGuesses">
            ...
            >
                <div role="rowheader" class="guess-table-cell guess-table-monster-cell">
                    <img
                        :src="getLatestIconForMonster(guess.monsterCode)"
                        class="table-guess-monster-icon"
                    />
                    <span class="guess-table-cell-content"
                        >{{ t(`game.monster.${guess.monsterCode}.name`) }}
                    </span>
                </div>
                <div
                ...
                >
                    <span class="guess-table-cell-content">{{
                        t(getEnumTranslationKey(Classifications, guess.criterias.classification))
                    }}</span>
                </div>
                <div
                ...
                >
                    <span
                        class="guess-table-cell-content"
                        v-if="guess.criterias.weaknesses.length < 1"
                    >
                        -
                    </span>
                ...
                >
                    <span
                        class="guess-table-cell-content"
                        v-if="guess.criterias.afflictions.length < 1"
                    >
                        -
                    </span>
                    <span class="guess-table-cell-content" v-else>
...
.accessibility-incorrect,
.accessibility-higher,
.accessibility-lower {
    background-image:
        repeating-linear-gradient(45deg, transparent, rgba(0, 0, 0, 0.2), transparent 5px 33%),
        repeating-linear-gradient(-45deg, transparent, rgba(0, 0, 0, 0.2), transparent 5px 33%);
}
...
.guess-table {
    ...
    .guess-table-row {
        display: grid;
        grid-auto-flow: row;
        grid-template-rows: minmax(30px, 1fr) // monster icon line
            repeat(6, minmax(60px, 0.8fr)); // criteria line
        gap: 0.1rem;
        ...
        .result-higher::before {
            clip-path: polygon(
                50% 0%,
                /* pointe en haut */ 100% 50%,
                /* coin droit milieu */ 70% 50%,
                /* coin droit du “corps” */ 70% 100%,
                /* bas droit */ 30% 100%,
                /* bas gauche */ 30% 50%,
                /* coin gauche du “corps” */ 0% 50% /* coin gauche milieu */
            );
        }
    }
}
